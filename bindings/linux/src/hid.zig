...
pub const HidDescriptor = packed struct {
    ...
    pub fn serialize(self: *const @This()) [9]u8 {
        ...
        out[2] = @as(u8, @intCast(self.bcd_hid & 0xff));
        out[3] = @as(u8, @intCast((self.bcd_hid >> 8) & 0xff));
        out[4] = self.country_code;
        out[5] = self.num_descriptors;
        out[6] = @intFromEnum(self.report_type);
        out[7] = @as(u8, @intCast(self.report_length & 0xff));
        out[8] = @as(u8, @intCast((self.report_length >> 8) & 0xff));
        return out;
    }
};
