...
pub fn new(id: []const u8, time: i64) @This() {
    return @This(){
        .id = id,
        .times = .{
...
pub fn addField(self: *@This(), field: Extension, time: i64, allocator: std.mem.Allocator) !void {
    self.fields = if (self.fields) |fields| blk: {
        ...
        var new_mem = try allocator.alloc(Extension, fields.len + 1);
        @memcpy(new_mem[0..fields.len], fields);
        allocator.free(fields);
        break :blk new_mem;
    } else blk: {
        break :blk try allocator.alloc(Extension, 1);
    };
    // At this point self.fields is NOT null

    var k = try allocator.alloc(u8, field.key.len);
    errdefer allocator.free(k);
    var v = try allocator.alloc(u8, field.value.len);
...
test "create Entry with UserName and URL fields" {
    ...
    try e.addField(.{ .key = "UserName", .value = "r4gus" }, time2, allocator);
    ...
    const x = try e.removeField("UserName", time6, allocator);
    defer {
...
test "serialize entry" {
    ...
    var e = @This().new(id, 0);
    defer e.deinit(allocator);
    try e.addField(.{ .key = "UserName", .value = "r4gus" }, 0, allocator);
    try e.addField(.{ .key = "URL", .value = "https://ziglang.org" }, 0, allocator);
