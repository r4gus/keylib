const std = @import("std");
const cbor = @import("zbor");
const Entry = @import("Entry.zig");

/// Name of the application that created this data
generator: []const u8,
/// Name of the database
name: []const u8,
/// Global times specific to all data
times: @import("Times.zig"),
/// List of database entries
entries: ?[]Entry = null,

pub fn new(generator: []const u8, name: []const u8, time: i64) @This() {
    return @This(){
        .generator = generator,
        .name = name,
        .times = .{
            .lastModificationTime = time,
            .creationTime = time,
            .lastAccessTime = time,
            .usageCount = 0,
        },
    };
}

pub fn deinit(self: *const @This(), allocator: std.mem.Allocator) void {
    if (self.entries) |entries| {
        for (entries) |entry| {
            entry.deinit(allocator);
        }
        allocator.free(entries);
    }
}

pub fn addEntry(self: *@This(), entry: Entry, time: i64, allocator: std.mem.Allocator) !void {
    self.entries = if (self.entries) |entries| blk: {
        for (entries) |_entry| {
            if (std.mem.eql(u8, _entry.id, entry.id)) {
                return error.DoesExist;
            }
        }

        var new_mem = try allocator.alloc(Entry, entries.len + 1);
        @memcpy(new_mem[0..entries.len], entries);
        allocator.free(entries);
        break :blk new_mem;
    } else blk: {
        break :blk try allocator.alloc(Entry, 1);
    };
    // At this point self.entries is NOT null

    self.entries.?[self.entries.?.len - 1] = entry;
    self.times.lastModificationTime = time;
}

pub fn getEntry(self: *@This(), id: []const u8, time: i64) ?*Entry {
    if (self.entries) |entries| {
        self.times.lastAccessTime = time;
        for (entries) |*entry| {
            if (std.mem.eql(u8, entry.id, id)) {
                return entry;
            }
        }
    }
    return null;
}

pub fn removeEntry(self: *@This(), id: []const u8, time: i64, allocator: std.mem.Allocator) !?Entry {
    if (self.entries) |entries| {
        var i: usize = 0;
        for (entries) |entry| {
            if (std.mem.eql(u8, entry.id, id)) {
                var e = entry;
                const end = self.entries.?.len - 1;
                var e_end = self.entries.?[end];
                self.entries = try allocator.realloc(self.entries.?, self.entries.?.len - 1);
                if (i < end) {
                    self.entries.?[i] = e_end;
                }
                self.times.lastModificationTime = time;
                return e;
            }
            i += 1;
        }
    }
    return null;
}

test "data tests" {
    _ = Entry;
}

test "create Data struct and add two entries" {
    const allocator = std.testing.allocator;
    const time = std.time.milliTimestamp();

    var d = @This().new("PassKeyXC", "DB1", time);
    defer d.deinit(allocator);
    try std.testing.expectEqual(time, d.times.lastModificationTime);
    try std.testing.expectEqual(time, d.times.creationTime);
    try std.testing.expectEqual(time, d.times.lastAccessTime);
    try std.testing.expectEqual(d.times.usageCount, 0);

    // Add first entry
    var id1 = try allocator.alloc(u8, 64);
    const time1 = std.time.milliTimestamp();
    std.crypto.random.bytes(id1[0..]);
    var e1 = Entry.new(id1, time1);
    try e1.addField(.{ .key = "UserName", .value = "r4gus" }, time1, allocator);
    try e1.addField(.{ .key = "URL", .value = "https://ziglang.org" }, time1, allocator);

    const time2 = std.time.milliTimestamp();
    try d.addEntry(e1, time2, allocator);
    try std.testing.expectEqual(time2, d.times.lastModificationTime);
    try std.testing.expectEqual(time, d.times.creationTime);
    try std.testing.expectEqual(time, d.times.lastAccessTime);
    try std.testing.expectEqual(d.times.usageCount, 0);
    try std.testing.expectEqual(@intCast(usize, 1), d.entries.?.len);

    // Add second entry
    var id2 = try allocator.alloc(u8, 64);
    const time3 = std.time.milliTimestamp();
    std.crypto.random.bytes(id2[0..]);
    var e2 = Entry.new(id2, time3);
    try e2.addField(.{ .key = "UserName", .value = "SugarYourCoffee" }, time1, allocator);
    try e2.addField(.{ .key = "URL", .value = "https://sugaryourcoffee.de" }, time1, allocator);

    const time4 = std.time.milliTimestamp();
    try d.addEntry(e2, time4, allocator);
    try std.testing.expectEqual(time4, d.times.lastModificationTime);
    try std.testing.expectEqual(time, d.times.creationTime);
    try std.testing.expectEqual(time, d.times.lastAccessTime);
    try std.testing.expectEqual(d.times.usageCount, 0);
    try std.testing.expectEqual(@intCast(usize, 2), d.entries.?.len);

    // Get the first entry
    const time5 = std.time.milliTimestamp();
    var e3 = d.getEntry(id1, time5);
    try std.testing.expect(e3 != null);
    try std.testing.expectEqualSlices(u8, "r4gus", e3.?.getField("UserName", time5).?);
    try std.testing.expectEqualSlices(u8, "https://ziglang.org", e3.?.getField("URL", time5).?);
    try std.testing.expectEqual(time4, d.times.lastModificationTime);
    try std.testing.expectEqual(time, d.times.creationTime);
    try std.testing.expectEqual(time5, d.times.lastAccessTime);
    try std.testing.expectEqual(d.times.usageCount, 0);
    try std.testing.expectEqual(@intCast(usize, 2), d.entries.?.len);

    // Remove first entry
    // e3 pointer is invalid after the removal!
    const time6 = std.time.milliTimestamp();
    var e4 = try d.removeEntry(id1, time6, allocator);
    try std.testing.expect(e4 != null);
    defer e4.?.deinit(allocator);
    try std.testing.expectEqualSlices(u8, "r4gus", e4.?.getField("UserName", time5).?);
    try std.testing.expectEqualSlices(u8, "https://ziglang.org", e4.?.getField("URL", time5).?);
    try std.testing.expectEqual(@intCast(usize, 1), d.entries.?.len);

    e3 = d.getEntry(id1, time6);
    try std.testing.expect(e3 == null);
    e3 = d.getEntry(id2, time6);
    try std.testing.expect(e3 != null);
    try std.testing.expectEqualSlices(u8, "SugarYourCoffee", e3.?.getField("UserName", time5).?);
    try std.testing.expectEqualSlices(u8, "https://sugaryourcoffee.de", e3.?.getField("URL", time5).?);

    // Remove second entry
    const time7 = std.time.milliTimestamp();
    var e5 = try d.removeEntry(id2, time7, allocator);
    try std.testing.expect(e5 != null);
    defer e5.?.deinit(allocator);
    try std.testing.expectEqualSlices(u8, "SugarYourCoffee", e5.?.getField("UserName", time5).?);
    try std.testing.expectEqualSlices(u8, "https://sugaryourcoffee.de", e5.?.getField("URL", time5).?);
    try std.testing.expectEqual(@intCast(usize, 0), d.entries.?.len);
}

test "serialize data" {
    const allocator = std.testing.allocator;

    var str = std.ArrayList(u8).init(allocator);
    defer str.deinit();

    var d = @This().new("PassKeyXC", "DB1", 0);
    defer d.deinit(allocator);

    var id1 = try allocator.alloc(u8, 64);
    @memcpy(id1, "");
    var e1 = Entry.new(id1, 0);
    try e1.addField(.{ .key = "UserName", .value = "r4gus" }, 0, allocator);
    try e1.addField(.{ .key = "URL", .value = "https://ziglang.org" }, 0, allocator);

    var id2 = try allocator.alloc(u8, 64);
    @memcpy(id2, "");
    var e2 = Entry.new(id2, 0);
    try e2.addField(.{ .key = "UserName", .value = "SugarYourCoffee" }, 0, allocator);
    try e2.addField(.{ .key = "URL", .value = "https://sugaryourcoffee.de" }, 0, allocator);

    try d.addEntry(e1, 0, allocator);
    try d.addEntry(e2, 0, allocator);

    try cbor.stringify(d, .{}, str.writer());

    try std.testing.expectEqualSlices(u8, "", str.items);
}
